...
class TimeAnalysisTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final times = months.map((m) {
      final totalMin = allRecords
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: CustomPaint(
                  painter: _TimeBarPainter(months: months, times: times),
                  child: Container(),
                ),
              ),
            ],
...
class _TimeBarPainter extends CustomPainter {
  ...
  _TimeBarPainter({required this.months, required this.times});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double maxTime =
    times.reduce((a, b) => a > b ? a : b).toDouble();
    if (maxTime == 0) maxTime = 1;

    for (int i = 0; i < n; i++) {
      final cx = (i * 2 + 1) * barW;
      final h = (times[i] / maxTime) * (size.height - 20);
